...
def _can_queue_sync(user) -> bool:
    return user.has_perm("netbox_unifi.run_sync") or user.has_perm(
        "netbox_unifi.add_syncrun"
    )


def _can_test_controller(user) -> bool:
    return user.has_perm("netbox_unifi.test_controller") or user.has_perm(
...
@login_required
@permission_required("netbox_unifi.view_syncrun", raise_exception=True)
def dashboard_view(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        if not _can_queue_sync(request.user):
            return HttpResponseForbidden(
                "Missing permission: netbox_unifi.run_sync or netbox_unifi.add_syncrun"
            )
        ...
        if form.is_valid():
            ...
            return redirect("plugins:netbox_unifi:dashboard")
    ...
    return render(request, "netbox_unifi/dashboard.html", context)


@login_required
@permission_required(
    "netbox_unifi.change_globalsyncsettings", raise_exception=True
)
def settings_view(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("plugins:netbox_unifi:settings")
        messages.error(request, "Unable to save settings. Fix validation errors.")

    return render(
        request,
        "netbox_unifi/settings.html",
        {"form": form, "settings": settings_obj},
    )


@login_required
@permission_required("netbox_unifi.view_unificontroller", raise_exception=True)
def controller_list_view(request: HttpRequest) -> HttpResponse:
    controllers = UnifiController.objects.order_by("name")
    return render(
        request, "netbox_unifi/controllers.html", {"controllers": controllers}
    )


@login_required
@permission_required("netbox_unifi.change_unificontroller", raise_exception=True)
def controller_edit_view(request: HttpRequest, pk: int | None = None) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("plugins:netbox_unifi:controllers")
    ...
    return render(
        request,
        "netbox_unifi/controller_form.html",
        {"form": form, "controller": controller},
    )


@login_required
@permission_required("netbox_unifi.delete_unificontroller", raise_exception=True)
def controller_delete_view(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        return redirect("plugins:netbox_unifi:controllers")
    return render(
        request, "netbox_unifi/controller_delete.html", {"controller": controller}
    )
...
def controller_test_view(request: HttpRequest, pk: int) -> HttpResponse:
    if not _can_test_controller(request.user):
        return HttpResponseForbidden(
            "Missing permission: netbox_unifi.test_controller or netbox_unifi.change_unificontroller"
        )
    controller = get_object_or_404(UnifiController, pk=pk)
    settings_obj = get_or_create_global_settings()
    try:
...
def controller_test_api_view(request: HttpRequest, pk: int) -> JsonResponse:
    if not _can_test_controller(request.user):
        return JsonResponse(
            {
                "status": "error",
                "error": "Missing permission: netbox_unifi.test_controller or netbox_unifi.change_unificontroller",
            },
            status=403,
        )
    controller = get_object_or_404(UnifiController, pk=pk)
...
@login_required
@permission_required("netbox_unifi.view_sitemapping", raise_exception=True)
def mapping_list_view(request: HttpRequest) -> HttpResponse:
    ...
    return render(request, "netbox_unifi/mappings.html", {"mappings": mappings})


@login_required
@permission_required("netbox_unifi.change_sitemapping", raise_exception=True)
def mapping_edit_view(request: HttpRequest, pk: int | None = None) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect("plugins:netbox_unifi:mappings")
        messages.error(request, "Unable to save mapping. Fix validation errors.")
    return render(
        request,
        "netbox_unifi/mapping_form.html",
        {"form": form, "mapping": mapping},
    )


@login_required
@permission_required("netbox_unifi.delete_sitemapping", raise_exception=True)
def mapping_delete_view(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        return redirect("plugins:netbox_unifi:mappings")
    return render(
        request, "netbox_unifi/mapping_delete.html", {"mapping": mapping}
    )
...
def run_detail_view(request: HttpRequest, pk: int) -> HttpResponse:
    ...
    return render(request, "netbox_unifi/run_detail.html", {"run": run})


@login_required
@permission_required("netbox_unifi.view_pluginauditevent", raise_exception=True)
def audit_list_view(request: HttpRequest) -> HttpResponse:
    ...
    return render(request, "netbox_unifi/audit.html", {"events": events})
