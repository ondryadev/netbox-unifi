...
def test_ping_ip_rejects_invalid_ip_without_subprocess():
    with patch("netbox_unifi.services.sync.ipam.subprocess.run") as run_mock:
...
def test_ping_ip_clamps_count_and_timeout():
    ...
    with patch("netbox_unifi.services.sync.ipam.subprocess.run", return_value=_Result()) as run_mock:
